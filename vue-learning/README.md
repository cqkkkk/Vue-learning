...
#### 7.各个多步骤的动画之间采用的是setTimeout这个方法来将全部步骤串连在一起，但是这样做的话每当计算一个步骤的延时时，要将前面的全部步骤的时间加在一起，越到后面加这个时间就越麻烦。swnb写了一个linkTimeout的类可以将setTimeout连在一起，具体用法待进一步理解。（已更新）
...
### 2018.09.23
#### 1.导入文件时路径问题：在其他项目中可以直接使用绝对路径但是到了该项目中却只能够使用绝对路径？？？（待进一步理解学习）
#### 2.导入文件时什么时候要加上大括号{}，什么时候不需要加？该项目中导入组件不加，导入方法要加？